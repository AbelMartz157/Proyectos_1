...
def ver_operaciones():
    cursor.execute("SELECT Proyecto, Ensamble, Operacion, Horario, HorasNormales, HorasExtra, Aprobadas FROM 'Datos Metalmecanica' WHERE Empleado = ? AND Fecha = ?", (text_empleado.get(), text_fecha.get()))
...
def aprobar_horas():
    result = messagebox.askyesno("Confirmación", "¿Deseas aprobar las horas capturadas?")
    if result:
        # Consulta SQL para actualizar la columna "Aprobados"
        sql = '''UPDATE "Datos Metalmecanica" 
                 SET Aprobadas = ? 
                 WHERE Fecha = ? AND Empleado = ?'''
        # Ejecutar la consulta con los valores
        cursor.execute(sql, ("Si", text_fecha.get(), text_empleado.get()))
        # Confirmar los cambios
        conexion.commit()
        messagebox.showinfo("Industria", "Horas aprobadas correctamente")
...
etiqueta_eliminar.place(x=1080, y=160)
etiqueta_aprobar = tk.Label(ventana, text="Aprobar", fg="white", font=("Tahoma", 12, "normal"), bg="#000040", width=7, height=1)
etiqueta_aprobar.place(x=995, y=160)

text_hrsEmpleado = tk.Entry(ventana, width=4, font=("Tahoma", 13), justify="center")
text_hrsEmpleado.place(x=1010, y=35) 
text_hrsEmpleadoExtras = tk.Entry(ventana, width=4, font=("Tahoma", 13), justify="center")
text_hrsEmpleadoExtras.place(x=1090, y=35) 
...
Imagen_aprobar = tk.PhotoImage(file="aprobar.png")
Aprobar = tk.Button(ventana, image=Imagen_aprobar, text="Borrar", command= aprobar_horas, width=50, height=50) 
Aprobar.place(x=1000, y=100)
...
Operaciones = ttk.Treeview(ventana, columns=(0, 1, 2, 3, 4, 5))
...
Operaciones.heading('#6', text='Aprobadas')
Operaciones.column('#6', width=100, anchor="center")
...
scroll_y.place(x=1157, y=230, height=203)
